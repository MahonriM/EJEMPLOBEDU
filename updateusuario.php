...
<?php
function actualizar_usuario($PNombreUsuario,$PContrasena,$PNombre,$PApellidoPaterno,$PIdRol,$PIdUsuario){
    $conn=mysqli_connect("localhost","root","","pia");
    $query=mysqli_prepare($conn,"CALL spupdateusuario(?,?,?,?,?,?)");
    mysqli_stmt_bind_param($query,"ssssii",$PNombreUsuario,$PContrasena,$PNombre,$PApellidoPaterno,$PIdRol,$PIdUsuario);
    $Res=mysqli_stmt_execute($query);
    if($Res!=1){
        echo"Nose ha modificado el registro verificar informacion";
    }
    else{
...
{
    actualizar_usuario($NombreUsuario,$Contrasena,$Nombre,$ApellidoPaterno,$IdRol,$IdUsuario);
}
...
<form action="updateusuario.php" method="post">
    ...
    <input type="text" name="txtusuario" pattern="[A-Za-z0-9_-]{1,15}" required>
    <br>
    ...
    <input type="text" name="txtnombre" pattern="[A-Za-z0-9_-]{1,15}" required>
    <br>
    <label for="">Ingresa el apellido paterno</label>
    <input type="text" name="txtapp" pattern="[A-Za-z0-9_-]{1,15}" required>
    <br>
